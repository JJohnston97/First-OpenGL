/// \File Mesh.cpp
/// \Breif Creates and sorts vertex and mesh information in VBO and VAO

//System Includes
#include <fstream>		//Include as a member of ostream
#include <iostream>		//Include for write to console for dbug 
#include <sstream>		//Include for reading strings
#include <vector>		//Include for use of vectors
//Project Includes
...
Mesh::Mesh()
{
	...
	m_VAO = 0;					 //VOA set to 0
	
	glGenVertexArrays(1, &m_VAO); //Creates one VAO

	m_numVertices = 0;			 //Number of vertice are set to 0
...
Mesh::~Mesh()
{
	...
	glDeleteVertexArrays(1, &m_VAO);		//Cleans up VOA
}
...
void Mesh::LoadOBJ(std::string filename)
{
	...
	std::ifstream inputFile(filename); //Find File
